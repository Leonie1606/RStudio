...
Gworte <- grep("^Ge|^ge", heinz, value=TRUE)


# A10 ----
# Finde alle Worte, die mit g enden.
worteG <- heinz[grepl("g$",heinz)]
...
anfangende_eingaben <- function(anfang, eingaben) return(eingaben[grepl(paste0("^",anfang),eingaben)])
...
abkuerzen <- function(eingabe) {return(gsub("viel.+Gerede","[...]",eingabe))}
