...
wort_existiert <- function(wort, liste) return(wort %in% liste)
...
welche_enthalten <- function(muster, eingaben) return(grep(muster,eingaben))
...
jeweils_enthalten <- function(muster, eingaben) return (grepl(muster,eingaben))
...
wort_existiert_case <- function(wort, liste) return(any(tolower(wort)==tolower(liste)))
