...
mehraugen <- factor(c(augenfarbe, "nussbraun"), levels = c(levels(augenfarbe), "nussbraun"))
mehraugen <- factor(c(as.character(augenfarbe), "nussbraun"), levels = c(levels(augenfarbe), "nussbraun"))
...
personenMitFarbe <- personenMitFarbe <- function(namen, farben) {
  haeufigste_farbe <- names(which.max(table(farben)))  # Häufigste Farbe ermitteln
  return(namen[farben == haeufigste_farbe])  # Personen mit dieser Farbe zurückgeben
}
...
mmmmEinkommen <- tapply(einkommen,augenfarbe, function(x) c(min(x),median(x),mean(x),max(x)))
mmmmEinkommen
# $blau
#   1500   1750   1750   2000 
# $braun
...
# tb_start
mmmmEinkommenKurz<- mmmmEinkommenKurz <- tapply(einkommen, augenfarbe, summary)
