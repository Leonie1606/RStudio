...
insurances <- read.csv("R33d6_insurances.csv", na.strings = c("", "NA"))
insurances$n_insurances <- as.integer(insurances$n_insurances)
...
tage <- read.fwf("R33d7_tage.txt", widths = c(4,-1,2,-1, 2), colClasses = "character",skip=0)
...
# A8 ----
data(iris)  ;  head(iris)
write.table(iris, file = "iris.txt", sep = "\t", quote = FALSE, row.names = FALSE, dec = ",")
