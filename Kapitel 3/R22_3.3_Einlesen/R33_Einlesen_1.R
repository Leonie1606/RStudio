...
mortalities <- read.table("R33d1_mortalities.txt", header=TRUE, stringsAsFactors=FALSE, nrows=15)
...
diseases <- read.table("R33d2_diseases.txt", header=TRUE, sep="\t", stringsAsFactors=FALSE)

# A3 ---- # Auch in den nächsten Aufgaben ist der Objektname im Dateinamen enthalten. 
# Nutze die Rstudio Autocompletion (Tab taste) zu "expe" und mach nie wieder Tippfehler :) 
# Hinweis: dafür muss der Mauszeiger (Cursor) innerhalb der Anführungsstriche sein. # Tipp: Beim Dateiinhalt anzeigen auch mal runterscrollen und auf Dezimalzeichen achten.
# Tipp: manche Daten haben Metadaten, aber keine Spaltennamen. 
expenditure <- read.delim("R33d3_expenditure.txt",header=FALSE, sep = "", stringsAsFactors=FALSE, skip=7, nrows=194, dec=",",col.names = c("V1", "V2", "V3", "V4"),colClasses = c("character", "numeric", "numeric", "numeric"))

# A4 ---- 
mortality <- read.table("R33d4_mortality.txt", header=TRUE, sep=";", stringsAsFactors=FALSE, nrows=15, skip=4)
