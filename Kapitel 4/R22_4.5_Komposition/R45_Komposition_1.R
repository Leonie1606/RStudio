...
par(
  mar = c(3, 3, 2, 0.5),  # Ränder (unten, links, oben, rechts)
  mgp = c(1.8, 0.7, 0),   # Position der Achsenbeschriftungen und Linien
  las = 1,                # Alle Zahlen aufrecht
  bg = "rosybrown"        # Hintergrundfarbe
)

# A2 ----
# Stelle nun 500 normalverteilte Zufallszahlen in einem Punktdiagramm dar.
# t2_start
plot(rnorm(500), pch = 16, col = "blue", main = "Normalverteilte Zufallszahlen")
# t2_ende
...
# A3 ----
# Korrigierte Definition von survival
# A3 ----
# Korrigierte Definition von survival
# A3 ----
# Korrigierte Definition von survival
survival <- as.data.frame(Titanic["Female", "Adult", , ])  # Entferne überflüssige Dimensionen
colnames(survival) <- c("Class", "Survived", "Freq")  # Spalten sinnvoll benennen

# Umwandlung von Freq in eine numerische Matrix
survival$Freq <- as.numeric(survival$Freq)  # Sicherstellen, dass Freq numerisch ist
survival_matrix <- xtabs(Freq ~ Class + Survived, data = survival)  # Frequenzen berechnen
survival_matrix <- as.matrix(survival_matrix)  # Umwandlung in eine echte Matrix
...
barplot(
  ...
  horiz = TRUE,  # Waagerechte Balken
  ylim = c(0, 15),  # Y-Achsenbereich von 0 bis 15
  main = "Weibliche Erwachsene auf der Titanic",  # Titel
  las = 1,  # Achsenbeschriftungen aufrecht
  col = c("red", "blue"),  # Farben für die Balken
  legend.text = colnames(survival_matrix)  # Legende basierend auf Überlebensstatus
)
...
plot(chickwts$weight, type = "l", lwd = 4, col = "violetred", main = "Gewicht von Chickwts")
boxplot(weight ~ feed, data = chickwts, col = berryFunctions::divPal(6), las = 1, main = "Futter vs Gewicht")
